...
7. Mở `https://taydoautocar.vn/deploy-hook.php?check=1`. Kết quả đúng:
   ...
   ```json
   {"hook":"https-package-v1","envConfigured":true,"keyConfigured":true,"pharAvailable":true,"zlibAvailable":true,"symlinkAvailable":true}
   ```

Nếu `pharAvailable`, `zlibAvailable` hoặc `symlinkAvailable` là `false`, dừng lại và liên hệ hosting bật PHP Phar, zlib hoặc symlink; hook sẽ không thể giải nén hoặc kích hoạt Laravel an toàn.
...
4. Nén release, chia thành các chunk 4 MiB, ký HMAC từng chunk.
...
ADMIN_INITIAL_PASSWORD=<mật khẩu mạnh ít nhất 12 ký tự>
...
- HTTP 413: tăng `upload_max_filesize` và `post_max_size` lên ít nhất 8 MiB trong PHP Settings.
