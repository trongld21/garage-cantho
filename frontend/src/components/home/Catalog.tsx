...
export function ProductCatalog({ products, searchable = false, initialCategory = 'all', initialSearch = '', collections = false }: { products: ProductItem[]; searchable?: boolean; initialCategory?: string; initialSearch?: string; collections?: boolean }) {
  ...
  return <div>
    ...
    <div className="auto-product-grid">{visible.map(product => <a key={product.id} href={`/store/${product.slug}/`} className="auto-product-card"><div className="auto-product-image"><ImageWithFallback src={product.image} alt={product.name} loading="lazy" fallbackType="product" />{product.sale_price != null && product.sale_price < product.price && <span className="auto-sale">-{Math.round((1 - product.sale_price / product.price) * 100)}%</span>}<span className="auto-card-arrow"><ArrowUpRight size={18} /></span></div><div className="auto-product-body"><p className="auto-card-meta">{product.brand} / {product.category}</p><h3>{product.name}</h3><div className="auto-price">{money(product.sale_price ?? product.price)}{product.sale_price != null && <del>{money(product.price)}</del>}</div><p className="auto-stock">{product.stock > 0 ? 'Còn hàng · Liên hệ tư vấn lắp đặt' : 'Liên hệ đặt hàng'}</p></div></a>)}</div>
...
export function CarCatalog({ cars, initialBrand = 'all', initialType = 'all' }: { cars: CarItem[]; initialBrand?: string; initialType?: string }) {
  ...
  return <><div className="auto-catalog-toolbar"><div className="auto-tabs" aria-label="Lọc hãng xe">{['all', ...brands].map(b => <button key={b} onClick={() => setBrand(b)} aria-pressed={brand === b} className={brand === b ? 'active' : ''}>{b === 'all' ? 'Tất cả hãng xe' : b}</button>)}</div><label className="auto-sort">Nhu cầu <select value={type} onChange={e => setType(e.target.value)}><option value="all">Tất cả xe</option><option value="sale">Mua xe</option><option value="rent">Thuê xe</option></select></label></div><div className="auto-product-grid">{visible.map(car => <a className="auto-car-card" key={car.id} href={`/cars/${car.slug}/`}><div className="auto-car-image"><ImageWithFallback src={car.image} alt={car.title} loading="lazy" /><span className="auto-car-type">{car.listing_type === 'sale' ? 'XE MUA BÁN' : 'XE CHO THUÊ'}</span></div><div className="auto-product-body"><p className="auto-card-meta">{car.year} · {car.transmission} · {car.mileage}</p><h3>{car.title}</h3><div className="auto-price">{money(car.price)}{car.listing_type === 'rent' && <small>/ ngày</small>}<ArrowUpRight size={20} /></div></div></a>)}</div>{visible.length === 0 && <div className="auto-empty"><p>Chưa có xe phù hợp với lựa chọn này.</p><button className="auto-button" onClick={() => { setBrand('all'); setType('all'); }}>Xem tất cả xe</button></div>}</>;
}
