...
export function AdminGate({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (normalizedPath === '/admin/login') return;
    let active = true;
    adminAuth.me().then(user => {
      if (!active) return;
      if (user.must_change_password && normalizedPath !== '/admin/account') { router.replace('/admin/account'); return; }
      setSession({ path: normalizedPath, user });
      setError('');
    }).catch(err => {
      if (!active) return;
  ...
  if (!session || session.path !== normalizedPath) return <div className="p-10 text-white">{error ? <><p role="alert">{error}</p><button onClick={() => window.location.reload()} className="mt-4 underline">Thử lại</button></> : 'Đang kiểm tra phiên đăng nhập…'}</div>;
