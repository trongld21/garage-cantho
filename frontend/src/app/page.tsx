...
export default async function HomePage() {
  ...
  return <div className="auto-home">
    ...
    <section className="auto-service-directory"><div className="auto-container"><div><p className="auto-eyebrow">DỊCH VỤ Ô TÔ</p><h2>NÂNG CẤP TIỆN NGHI<br />CHO CHIẾC XE CỦA BẠN</h2><p>Tìm dịch vụ phù hợp và trao đổi trực tiếp với đội ngũ Tây Đô.</p></div><div className="auto-service-links">{services.map(service => <Link key={service.id} href={`/services/${service.slug}`}><Wrench size={20} /><span>{service.name}</span><ArrowUpRight size={18} /></Link>)}</div></div></section>
